...
class PromptConfig:
    ...
    include_output_instructions: bool = True  # Include output format instructions
...
class PhilosophyExtractionPromptGenerator(IPromptGenerator):
    ...
    def generate(
        ...
        try:
            ...
            if self.config.include_output_instructions:
                prompt = self._add_output_format_instructions(prompt, config)
...
"""
    ...
    def _get_format_instructions(
        self, output_format: str, config: PhilosophyExtractorConfig
    ) -> str:
...
arguments,"premise1;conclusion1",0.8"""
...
- **Fields Extracted:** main_thesis, key_concepts, arguments"""
...
| arguments | premise1 -> conclusion1 | 0.8 |"""
        ...
        elif output_format == "custom" and config.custom_format_template:
            return f"""
OUTPUT FORMAT: CUSTOM
- Follow this custom template exactly:
{config.custom_format_template}

IMPORTANT: Use the exact structure and formatting specified in the custom template."""
...
"""
        ...
        if self.prompt_generator.config.include_output_instructions:
            # Create a temporary config with output format for instructions
            temp_config = PhilosophyExtractorConfig(
                source_type=config.source_type,
                language=config.language,
                extraction_depth=config.extraction_depth,
                target_audience=config.target_audience,
                extraction_mode=config.extraction_mode,
                categories_focus=config.categories_focus,
                output_format=OutputFormat(output_format),
            )
            # Set custom format template separately if provided
            if custom_format_template:
                temp_config.custom_format_template = custom_format_template
            prompt += self.prompt_generator._add_output_format_instructions(
                "", temp_config
            )
